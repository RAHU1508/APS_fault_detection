...
from scipy.stats import ks_2samp  #this will compare the statistics between 2 datasets & returns the p value, if p value <0.05 means distribution of particular column between 2 datasets is statistically different
...
class DataValidation:
    ...
    def __init__(self,
                    ...
                    data_ingestion_artifact:artifact_entity.DataIngestionArtifact):  #data_ingestion_artifact is the o/p from data ingestion component which will be an input to the data validation component
        try:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:  #using Option means it cannot return dataframe, this will be the case when all the column records is null 
        """
        ...
        try:
            ...
            threshold = self.data_validation_config.missing_threshold #missing_threshold is defined in config_entity, its value can be changed, here 0.2 is considered
    ...
    def is_required_columns_exists(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool: 
        try:
            ...
            missing_columns = []  #all the features which is present in the base df & not in gthe current df will be listed here
            ...
            if len(missing_columns)>0:
                self.validation_error[report_key_name]=missing_columns # that list will be replicated in the validation_error in the form of dict. see dict data type in variable definition above
                return False
            return True
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df = pd.read_csv(self.data_validation_config.base_file_path) #base df filepath is defined in config_entity file
